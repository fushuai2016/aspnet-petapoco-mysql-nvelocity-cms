    ...
    public partial class manager_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind(CombSqlTxt(keywords), " ORDER BY AddTime DESC", model);
            }
        }

        #region 数据绑定=================================
        private void RptBind(string _strWhere, string _orderby, system_user model)
        {
            this.page = DTRequest.GetQueryInt("page", 1);
            txtKeywords.Text = this.keywords;
            if (!string.IsNullOrEmpty(this.keywords) && model.UserType != 3)
            {
                this.rptList.DataSource = db.GetPageList<system_user>(this.page, this.pageSize, _strWhere + " AND SiteID=" + siteConfig.SiteID, _orderby, out this.totalCount);
            }
            else 
            {
                this.rptList.DataSource = db.GetPageList<system_user>(this.page, this.pageSize, _strWhere, _orderby, out this.totalCount);
            }
        ...
        protected string CombSqlTxt(string _keywords)
        {
            ...
            {
                strTemp.Append(" (UserName like  '%" + _keywords + "%' OR RealName like '%" + _keywords + "%' OR EnglishName like '%" + _keywords + "%')");
            }

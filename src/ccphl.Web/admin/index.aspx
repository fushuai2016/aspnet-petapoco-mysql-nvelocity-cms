<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="index.aspx.cs" Inherits="ccphl.Web.admin.index" %>
...
<script type="text/javascript">
    ...
    function loadMenuTree(_islink) {
        ...
        $.ajax({
            type: "post",
            url: "../tools/admin_ajax.ashx?action=get_navigation_list&siteid="+$("#sid").val()+"&time=" + Math.random(),
            dataType: "html",
    ...
    function initMenuTree(islink) {
        ...
        $("#sidebar-nav .list-box .list-group").each(function (i) {
            //添加菜单导航
            var navHtml = $('<li><i class="' + $(this).attr("icon") + '"></i><span>' + $(this).attr("name") + '</span></li>').appendTo($("#nav"));
            //默认选中第一项
            if (i == 0) {
                $(this).show();
...
<asp:HiddenField ID="sid" ClientIDMode="Static" runat="server" />
<!--全局菜单-->
